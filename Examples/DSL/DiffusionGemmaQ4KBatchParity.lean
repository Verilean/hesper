...
/-! Batched Q4_K matmul parity: validates `fusedQ4KMBatchKernel` (M rows in one dispatch)
    against the ggml-validated single-row `fusedQ4KMLinearKernel` run per row. -/
open Hesper.WebGPU

def main : IO Unit := do
  let inD := 256; let outD := 8; let M := 4
  IO.println "[dg-q4kbatch-parity] init WebGPU (Metal)..."
  let inst ← Hesper.init
  let device ← getDevice inst
  let q4 ← IO.FS.readBinFile "/tmp/dg_golden/q4kexp/a.bin"   -- expert-0 slab = [outD,inD] Q4_K
  ...
  let xs := (List.range (M*inD)).toArray.map (fun i => Float.sin (i.toFloat * 0.017) * 0.5)
  ...
  let kBatch := Hesper.Layers.Linear.fusedQ4KMBatchKernel { inDim := inD, outDim := outD } M
  ...
  Hesper.GPUBackend.execute device kBatch bufsB cfgB
  ...
  for m in [0:M] do
    ...
    let kS := Hesper.Layers.Linear.fusedQ4KMLinearKernel { inDim := inD, outDim := outD }
  ...
  IO.println s!"  batch[0..4] ={(ob.extract 0 4).toList}"
  IO.println s!"  single[0..4]={(ref.extract 0 4).toList}"
  if err < 1e-4 && ob.size == M*outD then
    IO.println "✓ batched Q4_K matmul matches per-row single kernel (ggml-validated) — N-row batching correct"
  else
    IO.println "✗ FAIL"; throw (IO.userError "batch parity failed")
