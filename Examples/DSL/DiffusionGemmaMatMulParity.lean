...
import Hesper.Basic
/-! DiffusionGemma f32 linear-projection parity vs ggml_mul_mat golden.
    (Q4_K-quantized matmul parity is validated at the GPU-kernel stage.)
    Run: ./scripts/llama_parity/dump_dg_matmul_golden /tmp/dg_golden/matmul
         lake exe diffusiongemma-matmul-parity -/
...
def main (args : List String) : IO Unit := do
  let dir := args.head?.getD "/tmp/dg_golden/matmul"
  let inDim := 24; let outDim := 16
  let a ← readF32Bin s!"{dir}/a.bin"
  let x ← readF32Bin s!"{dir}/x.bin"
  let gold ← readF32Bin s!"{dir}/out.bin"
  let out := matVec a x outDim inDim
  let mut err := 0.0
  for i in [0:outDim] do
    let d := (out[i]! - gold[i]!).abs
    if d > err then err := d
  IO.println s!"[dg-matmul-parity] in={inDim} out={outDim} maxAbsErr={err}"
  if out.size == gold.size && err < 1e-4 then IO.println "✓ PASS"
  else IO.println "✗ FAIL"; throw (IO.userError "matmul parity failed")
