...
/-!
# DiffusionGemma expert-indexed Q8_0 matmul parity on Metal

Validates `fusedQ8_0ExpertKernel` (selects expert `e` along the expert axis
of a `[inDim, outDim, nExpert]` Q4_K tensor, inline dequant) against the
ggml golden (dequant expert-slab × f32 input).

Run:
  ./scripts/llama_parity/dump_dg_q8exp_golden /tmp/dg_golden/q8exp
  lake exe diffusiongemma-q8exp-parity
-/
...
def main : IO Unit := do
  let dir := "/tmp/dg_golden/q8exp"
  let inD := 64; let outD := 8; let nExp := 3; let e := 1
  IO.println "[dg-q8exp-parity] init WebGPU (Metal)..."
  ...
  let kern := Hesper.Layers.Linear.fusedQ8_0ExpertKernel { inDim := inD, outDim := outD } nExp e
  ...
  if q.size == outD && err < 1e-3 then
    IO.println "✓ expert-indexed Q8_0 matmul on Metal matches ggml golden (expert selected, weights packed)"
  else
    IO.println "✗ FAIL"
    throw (IO.userError "q8exp parity failed")
