...
| [`16-shape-audit-checklist.md`](16-shape-audit-checklist.md) | Phase 1 shape audit (A/B/C).  **All 3 items done**; housekeeping 4-6 deferred.  Active checklist moved to doc 21. | `Hesper/Models/Gemma4.lean`, `Hesper/Layers/*.lean` |
...
| [`21-development-flow.md`](21-development-flow.md) | **Development flow + active checklist**: llama.cpp-first, golden-value-driven, **per-kernel unit tests BEFORE batch/fuse refactors**.  Includes the open per-kernel unit-test table. | `llama.cpp/common/debug.cpp:161` HESPER PATCH |
| [`22-golden-diff-status.md`](22-golden-diff-status.md) | **Golden-diff trail** that led to the PLE cache-key bug fix (commit `c3de2f7`).  Output went from "aga" garbage to English. | `Hesper/Models/Gemma4.lean` dumps |
