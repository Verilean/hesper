...
- **次の一手（TODO）**:
  ...
  6. [~] **M7 進行中（2026-07-05）: 8.05 → 68.0 t/s（8.4×）— まだ kernel チューニング前、全部オーバーヘッド除去**。llama.cpp 156.5 の 43%。
     ...
     - **次（順序確定、ユーザー裁定）**: **(1) mul_mv カーネル線を完遂** — llama.cpp `kernel_mul_mv_q4_K_f32` の内ループを ShaderM に**構造ごと忠実移植**（原則 7 center-point: 2 rows/sg × 2 sg/TG、simdgroup 内 4-block 並列 ix=tiisg/8、vectorized load、kmask scale 抽出）→ family の軸を拡張（vec-width / blocks-per-thread / smem staging / rows×sgs）→ sweep → **合否 = isolated ≥60% BW（native 測定）**。fused QKV/gateup/normGateUp kernel 群にも同 family 適用。(2) カーネル達成後に e2e 再測 → 残ギャップで B（native dispatch、デリスク済みパーク中）を再判定。(3) argmax readback 0.6ms / host record ~2ms は後段
