...
- **次の一手（TODO）**:
  ...
  6. [~] **M7 進行中（2026-07-05）: 8.05 → 68.0 t/s（8.4×）— まだ kernel チューニング前、全部オーバーヘッド除去**。llama.cpp 156.5 の 43%。
     ...
     - 残 TODO: (a) Tint バグ最小再現 → 回避形状で SWA 向け factors-less qkNormRope、(b) Q6K-V 層 K/V matmul 統合、(c) argmax readback（pipelined/deferred read）、(d) host record 残 ~1.5ms（ce 経路の bind/hash 単価）
