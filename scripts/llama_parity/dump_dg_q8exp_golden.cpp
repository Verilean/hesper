// DiffusionGemma expert-indexed Q8_0 golden: quantize [inDim, outDim*nExpert] to Q8_0,
// reference = dequant(expert e slab) · f32 input. Dumps full Q8_0 bytes + x + ref(expert e).
// Run: ./scripts/llama_parity/dump_dg_q8exp_golden /tmp/dg_golden/q8exp
#include <ggml.h>
#include <ggml-cpu.h>
#include <cmath>
#include <cstdio>
...
int main(int argc,char**argv){
  ...
  std::string o=argv[1];
  const int inD=64, outD=8, nExp=3, e=1;      // expert 1; inD%256==0
  ...
  struct ggml_tensor*a=ggml_new_tensor_2d(ctx,GGML_TYPE_Q8_0,inD,rows);
  ggml_quantize_chunk(GGML_TYPE_Q8_0, wf.data(), a->data, 0, rows, inD, nullptr);
  // dequant back to f32
  ...
  fprintf(stderr,"OK q8exp: inD=%d outD=%d nExp=%d e=%d bytes=%zu\n",inD,outD,nExp,e,ggml_nbytes(a));
  return 0;
}
